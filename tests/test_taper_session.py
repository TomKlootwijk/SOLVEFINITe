"""DP8-DP10 durable original contexts, producer retries and fresh continuation."""
...
from examples.taper_conformance import REFERENCE, ROOT, canonical_hash, scenario_for
from solvefinite.f8 import IndexBinding
from solvefinite.field_agent import TAPER_POLICY
...
def advance_reference(live, first=0, stop=11):
...
class TaperSessionTests(unittest.TestCase):
    def setUp(self):
        temporary=TemporaryDirectory(prefix="taper-session-")
    ...
    def test_split_session_preserves_original_context_and_readonly_inspection(self):
        ...
        self.assertEqual((complete["status"],complete["cycle"],complete["state"]["energy"]),("COMPLETE",11,69))
        configured,restored=load_session(self.path,capacity=12)
        try:
            self.assertEqual(configured,scenario_for())
            self.assertEqual(f"{restored.agent_pair:016X}","16000534060005CC")
    ...
    def test_two_generations_retain_distinct_original_contexts(self):
        ...
        self.assertEqual((result["status"],result["cycle"],result["state"]["geometry_epoch"],
                          result["state"]["energy"]),("COMPLETE",17,2,55))
    ...
    def test_live_stale_epoch_is_rejected_even_when_topology_and_locality_match(self):
        ...
        with LiveSession(self.path,capacity=1,index_binding=IndexBinding(11,-1,0)) as reopened:
            ...
            self.assertEqual((final["state"]["cycle"],final["state"]["energy"]),(11,69))
    ...
    def test_fresh_frames_and_new_hazards_cannot_replace_grammar_or_world(self):
        with LiveSession(self.path,config=LiveConfig(scenario_for().manifest)) as live:
            ...
            bad=observe_request(moved);bad["taper"]={}
    ...
    def test_cli_exports_and_continues_taper_profile(self):
        configured,live_config=self.directory/"scenario.json",self.directory/"live.json"
        commands=[
            ["scenario","--profile","taper","--output",str(configured)],
            ["live-config","--profile","taper","--output",str(live_config)],
        ...
        self.assertEqual((final["state"]["geometry_epoch"],final["state"]["energy"]),(1,69))
        self.assertEqual(json.loads(live_config.read_text())["agent"]["policy"],TAPER_POLICY)
...
class RealTaperContinuationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_fresh_process_crossovers_on_each_side_of_admitted_generation(self):
        with TemporaryDirectory(prefix="taper-crossover-") as temporary:
            ...
            for source,target in (("cpu","gpu"),("gpu","cpu")):
                for cut in (4,5):
                    ...
                    self.assertEqual((final["status"],final["cycle"],final["state"]["energy"]),("COMPLETE",11,69))
    ...
    def test_fresh_process_preserves_generated_world_defer_and_original_tick(self):
        with TemporaryDirectory(prefix="taper-defer-") as temporary:
