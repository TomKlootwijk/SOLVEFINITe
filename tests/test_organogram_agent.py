"""OG8 independent one-owner missions, context identity and candidate failures."""
...
def empty_boundary_manifest():
    ...
    value.update(axiom=[{"symbol":"R","args":[127]}, {"symbol":"S","args":[]}],
                 generations=0, rules=[])
    return scenario_for(organogram=OrganogramBinding.from_dict(value)).manifest


class OrganogramManifestTests(unittest.TestCase):
    def test_exact_new_manifest_and_preserved_legacy_shapes(self):
        manifest = scenario_for().manifest
        value = manifest.to_dict()
        self.assertEqual(set(value), set(FieldAgentManifest().to_dict()) | {"routing", "organogram"})
        self.assertEqual(AgentManifest.from_dict(value), manifest)
        for key in ("routing", "organogram"):
        ...
        for key, extra in (("growth", {"format":"klein-dyadic-growth-v1","max_epochs":1,"cost":1}),
            ...
            with self.subTest(key=key), self.assertRaises(ValueError):
                AgentManifest.from_dict(damaged)
        for policy in (FIELD_POLICY, HADAMARD_POLICY, GROWTH_POLICY):
            previous = FieldAgentManifest(policy=policy).to_dict()
            self.assertNotIn("organogram", previous)
            self.assertEqual(AgentManifest.from_dict(previous).to_dict(), previous)
            with self.assertRaises(ValueError):
                FieldAgentManifest(policy=policy, organogram=manifest.organogram)
    ...
    def test_binding_is_retained_and_not_shared_mutable_json(self):
        ...
        value["organogram"]["rules"][0]["rhs"][0]["symbol"] = "S"
        value["organogram"]["limits"]["max_steps"] = 1
        self.assertEqual(manifest.to_dict(), expected)


class OrganogramAgentTests(unittest.TestCase):
    backend = "cpu"
    ...
    def test_owner_is_preserved_while_hypothetical_branch_cursor_changes(self):
        ...
        for name, binding in (("nested", REFERENCE["binding"]), ("unbranched", unbranched)):
            with self.subTest(grammar=name):
                manifest = scenario_for(organogram=OrganogramBinding.from_dict(binding)).manifest
    ...
    def test_wait_changes_original_tick_and_production_without_resetting_history(self):
        ...
        self.assertNotEqual(expected["document"]["tape"],REFERENCE["standalone_stage"]["document"]["tape"])
    ...
    def test_pure_empty_boundary_rejection_keeps_exact_usable_owner(self):
        ...
        # Radius 127 exceeds this entire quotient, so every union potential
        # is negative and there is no boundary. No fault injection is needed.
        for _ in range(2):
            with self.assertRaises(ValueError):
                agent.step(frame(agent))
    ...
    def test_rejected_complete_candidate_preserves_owner_and_records_resource_peak(self):
        ...
        with patch("solvefinite.tomigidt.select_organogram_target",
    ...
    def test_strict_replay_rejects_context_rule_and_receipt_substitution(self):
        ...
        changes = (
            ...
            lambda a:a["events"][4]["growth"]["recipe"]["organogram"]["rules"][0]["guards"][0].update(value=5),
    ...
    def test_same_quantum_defer_survives_each_reindex_and_cache_capacity(self):
        ...
        self.assertNotEqual(a["stages"][0]["expected_derivation"]["tape"],
                            REFERENCE["standalone_stage"]["document"]["tape"])
...
class OrganogramOwnerFailureTests(unittest.TestCase):
...
class RealOrganogramAgentTests(OrganogramAgentTests):
    backend="gpu"
