"""Capture OG1-OG8 against the frozen preimplementation formal reference."""
...
DEST = ROOT / 'docs/evidence/organogram-v1'
FORMAL_COMMIT = '5c76f2cec3a1cec8fd2d16eeeac2021c45b0aea9'
BASE_COMMIT = '94f86c7ae84a6eee9b99d3101d3b531b7a307c17'
...
def main():
    ...
    print('Running independent-reference CPU/GPU organogram conformance.', flush=True)
    conform_started = perf_counter()
    run(['-m', 'examples.organogram_conformance', '--output', str(DEST / 'conformance.json')])
    ...
    if (not conformance['checks'] or any(value is not True for value in conformance['checks'].values())
            or conformance['actual_GPU_capture'] is not True):
        raise ValueError('Every actual-device conformance check must pass')

    print('Capturing fresh-process GPU -> CPU -> GPU growth and inspection.', flush=True)
    scratch = ROOT / 'output/organogram/cli' / datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%f')
    scratch.mkdir(parents=True)
    scenario, state = scratch / 'scenario.json', scratch / 'session.json'
    commands = [
        ['-m', 'solvefinite', 'agent', 'scenario', '--profile', 'organogram', '--output', str(scenario)],
    ...
    commands.append(['-m', 'solvefinite', 'agent', 'inspect', str(state), '--backend', 'cpu'])
    outputs.append(json.loads(run(commands[-1]).stdout))
    if (outputs[1]['state']['status'] != 'GROWTH_PENDING'
            or outputs[2]['state']['geometry_epoch'] != 1
            or outputs[-1]['state'] != conformance['GPU']['default']['archive']['expected']
            or state.read_bytes() != before):
        raise ValueError('Fresh process growth, final state or read-only inspection failed')
    ...
    report = {
        'format': 'organogram-verification-v1', 'captured_utc': datetime.now(timezone.utc).isoformat(),
        'formal_binding_commit': FORMAL_COMMIT, 'base_commit': BASE_COMMIT,
        'preimplementation_formal_pdf_sha256': digest(formal_pdf),
        ...
        'commands': ['python -m unittest discover -s tests -v',
                     'python -m examples.organogram_conformance --output docs/evidence/organogram-v1/conformance.json',
                     'Fresh process commands retained in cli-replay.json'],
        'remaining_obligations': ['Arbitrary cone/pyramid and graph productions',
                                  'Global spectral choices', 'WElip and clock continuation',
                                  'Physical adapters', 'Comparative hardware measurements'],
        'scope': 'Finite OG1-OG8 correctness and continuation evidence; no universality, throughput, cache saturation or physical-energy claim.',
    }
