"""Capture DP1-DP10 against the frozen independent reference and an actual GPU.

Run ``python -m examples.taper_conformance --output output/organogram.json``.
...
from examples.organogram_conformance import (no_cpu_compilers as no_og_cpu_compilers,
    mission as og_mission, scenario_for as og_scenario_for)
from solvefinite.f8 import IndexBinding
from solvefinite.field_agent import (
    FIELD_POLICY, HADAMARD_POLICY, GROWTH_POLICY, TAPER_POLICY, FieldAgentManifest,
)
...
FORBIDDEN_TAPER_PRODUCERS = (
    "solvefinite.taper.interpret_cpu", "solvefinite.taper.regenerate",
    "solvefinite.taper.occupancy_cpu", "solvefinite.taper.signs_from_occupancy_cpu",
    "solvefinite.taper.union_signs_cpu", "solvefinite.taper.evaluate_field",
    "solvefinite.taper.TaperFieldRecipe.field_manifest", "solvefinite.tomigidt.regenerate_taper",
)
_ORACLE = None
...
def check(condition, label):
    if type(condition) is not bool or not condition:
        raise ValueError(f"Taper conformance failed: {label}")
    return True
...
def oracle():
    ...
    if _ORACLE is None:
        spec = importlib.util.spec_from_file_location(
            "independent_DP_conformance_oracle", REFERENCE_PATH.with_name("reference-builder.py"))
...
def scenario_for(reference=None, **changes):
    ...
    values = dict(policy=TAPER_POLICY, world=recipe,
                  taper=TaperBinding.from_dict(reference["binding"]),
...
def no_cpu_compilers():
    ...
    for name in FORBIDDEN_TAPER_PRODUCERS:
        stack.enter_context(patch(name,
            side_effect=AssertionError(f"Forbidden CPU taper producer: {name}")))
    return stack
...
def expected_stage(agent, prior_fields):
    ...
    world = oracle().OG.Field(base.width, base.height, base.center, base.radius, values=prior_fields)
    return oracle().interpret(world, agent.manifest.taper.to_dict(), context)
...
def assert_stage(agent, expected, old_recipe, old_pair):
    ...
    check(receipt["format"] == "klein-taper-growth-v1", "receipt profile")
    ...
    check(recipe["base"] == agent.manifest.world.to_dict()
          and recipe["taper"] == agent.manifest.taper.to_dict()
    ...
    next_world = oracle().OG.Field(agent.current_recipe.width, agent.current_recipe.height,
                               values=expected["union"]["field"])
    target = oracle().OG.target_selection(next_world, g, (-r if a & 16 else r) & 255)
    check(agent.target == target["target_path"] == receipt["target"], "independent DP target")
...
def cpu_conformance():
    ...
    og = og_mission(og_scenario_for(), "cpu")
    retained_og = json.loads((ROOT / "docs/evidence/organogram-v1/verification.json").read_text())
    check(og["archive_sha256"] == retained_og["canonical_archive_sha256"], "legacy OG archive")
    legacy["organogram"] = {"archive": og["archive"], "archive_sha256": og["archive_sha256"]}
...
import json,sys
from examples.taper_conformance import no_cpu_compilers,canonical_hash,MAX_CYCLES
from solvefinite.f8 import IndexBinding
from solvefinite.session import Scenario
from solvefinite.tomigidt import Tomigidt
...
def demonstrate(cpu_only=False):
    ...
    checks = {name: True for name in (
        ...
        "legacy_FI_HP_GD_OG_archives_preserved")}
    report = {"format": "taper-conformance-v1", "formal_binding": "TK-LPLUT-2.0 revision13 DP1-DP10",
    ...
    if not cpu_only:
        ...
        replays = {}
        with TemporaryDirectory(prefix="solvefinite-taper-") as temporary:
        ...
        report.update(GPU=gpu, GPU_deferred=deferred, fresh_process_replays=replays, actual_GPU_capture=True,
                      ...
                      disabled_taper_CPU_producers=list(FORBIDDEN_TAPER_PRODUCERS))
    ...
    report.update(canonical_archive_sha256=cpu["cases"]["default"]["archive_sha256"],
                  ...
                  scope="Finite directional sections, exact projected-occupancy fields and autonomous continuation. "
...
def main():
    ...
    parser.add_argument("--output", default="output/taper-conformance.json")
